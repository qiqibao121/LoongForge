...
class FP8GranularityState(PowerSGDState):
    """Software FP8-like gradient communication state for granularity tests.

    This is deliberately a reference implementation for the A800 precision
    experiment. It quantizes each DDP gradient tensor independently, uses
    exact fallback for 1-D or <=64 KiB tensors, and keeps error feedback in
    the existing bucket-sized residual buffer. It does not alter parameter
    all-gather/broadcast because those collectives are not exposed by this
    DDP hook path.
    """

    def __init__(self, *args, granularity: str = "block", min_bytes: int = 65536,
                 block_size: int = 128, log_path: str = "", **kwargs):
        super().__init__(*args, **kwargs)
        if granularity not in {"row", "column", "block"}:
            raise ValueError("FP8 granularity must be row, column, or block")
        self.granularity = granularity
        self.min_bytes = int(min_bytes)
        self.block_size = int(block_size)
        self.log_path = log_path or os.getenv("FP8_GRANULARITY_LOG", "")
        self.logged = set()
...
def _fp8_granularity_quantize(tensor: torch.Tensor, mode: str, block_size: int):
    """Return uint8 payload, FP32 scales, and original shape."""
    flat = tensor.reshape(-1).float()
    shape = tuple(tensor.shape)
    if mode == "row":
        groups = tensor.reshape(tensor.shape[0], -1).float()
    elif mode == "column":
        groups = tensor.reshape(-1, tensor.shape[-1]).float().transpose(0, 1)
    elif mode == "block":
        padded = ((flat.numel() + block_size - 1) // block_size) * block_size
        groups = torch.nn.functional.pad(flat, (0, padded - flat.numel()))
        groups = groups.reshape(-1, block_size)
    else:
        raise ValueError(mode)
    scale = groups.abs().amax(dim=1).clamp_min(1e-12) / 448.0
    q = (groups / scale[:, None]).clamp(-448.0, 448.0).to(torch.float8_e4m3fn)
    return q.contiguous().view(torch.uint8), scale, shape


def _fp8_granularity_dequantize(qbytes, scale, mode, shape, block_size):
...
def _fp8_granularity_reduce_view(
    view: torch.Tensor,
    mode: str,
    block_size: int,
    world_size: int,
    ...
    """Quantize/reduce a tensor in bounded chunks.

    This keeps only a chunk-sized FP16/BF16 workspace and uses an exact
    all-reduce on the dequantized chunk.  It is intentionally a precision
    reference path (communication volume is not representative of a fused
    production kernel), but avoids materializing all-rank dequantized copies.
    """
    if view.ndim != 2:
        flat = view.reshape(-1)
        chunk_elems = 1 << 20
        for start in range(0, flat.numel(), chunk_elems):
            stop = min(start + chunk_elems, flat.numel())
            part = flat[start:stop]
            q, s, shape = _fp8_granularity_quantize(
                part, "block", block_size
            )
            deq = _fp8_granularity_dequantize(
                q, s, "block", shape, block_size
            ).to(view.dtype).contiguous()
            dist.all_reduce(deq, group=group)
            part.copy_(deq.div_(world_size))
        return
    rows, cols = view.shape
    if mode == "row":
        chunk_rows = max(1, min(rows, (1 << 24) // max(cols, 1)))
        for r in range(0, rows, chunk_rows):
            part = view[r:min(r + chunk_rows, rows)]
            q, s, shape = _fp8_granularity_quantize(part, "row", block_size)
            deq = _fp8_granularity_dequantize(
                q, s, "row", shape, block_size
            ).to(view.dtype).contiguous()
            dist.all_reduce(deq, group=group)
            part.copy_(deq.div_(world_size))
    elif mode == "column":
        chunk_cols = max(1, min(cols, (1 << 24) // max(rows, 1)))
        for c in range(0, cols, chunk_cols):
            part = view[:, c:min(c + chunk_cols, cols)]
            q, s, shape = _fp8_granularity_quantize(part, "column", block_size)
            deq = _fp8_granularity_dequantize(
                q, s, "column", shape, block_size
            ).to(view.dtype).contiguous()
            dist.all_reduce(deq, group=group)
            part.copy_(deq.div_(world_size))
    elif mode == "block":
...
def fp8_granularity_hook(
    state: FP8GranularityState,
    bucket: dist.GradBucket,
) -> torch.futures.Future[torch.Tensor]:
    """Reference row/column/block FP8-like gradient hook.

    Each tensor is gathered with its local scale and averaged after
    dequantization. Small or 1-D tensors use exact all-reduce. The
    implementation is intentionally synchronous and is only for short
    precision smoke runs, not production throughput.
    """
    group = state.process_group if state.process_group is not None else dist.group.WORLD
    world_size = dist.get_world_size(group)
    input_tensor = bucket.buffer()
    ...
    if residual is None:
        ...
        state.error_dict[bucket_index] = residual
    residual.add_(input_tensor)
    # Operate directly on the residual to avoid a second full-bucket clone.
    corrected = residual
    offset = 0
    selected = 0
    selected_bytes = 0
    for tensor in bucket.gradients():
        ...
        use_quant = tensor.ndim > 1 and tensor.numel() * tensor.element_size() > state.min_bytes
        if use_quant:
        ...
        offset += tensor.numel()
    input_tensor.copy_(corrected)
    residual.sub_(input_tensor)
    if dist.get_rank(group) == 0 and state.log_path and bucket_index not in state.logged:
        ...
        try:
            with open(state.log_path, "a", encoding="utf-8") as stream:
                stream.write(json.dumps({"iter": state.iter, "bucket": bucket_index,
    ...
    if (
        ...
        if is_fp8_granularity:
            state_kwargs.update(
                ...
                log_path=os.getenv("FP8_GRANULARITY_LOG", ""),
            )
        elif is_ef21_powersgd:
            state_kwargs.update(
